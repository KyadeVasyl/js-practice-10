...
function customAssign(...objects) {
  ...
  let mergedObj = Object.assign({}, ...objects);
  for (let value in mergedObj) {
    if (typeof mergedObj[value] === 'number') {
      mergedObj[value] = mergedObj[value].toString();
    }
  } return mergedObj;
...
function customObjectFromEntries(entries) {
  ...
  const preResult = entries.map(([key, value]) => {
  ...
  const result = Object.fromEntries(preResult);
...
function checkProperty(obj, prop) {
  ...
  if (typeof prop !== 'string' || prop === null) {
    return "Помилка: вхідний параметр не є рядком або невказаний.";
  }
...
function keysAndValues(obj) {
  ...
  return [keys, values]
  // ПІДКАЖІТЬ PLS ЯК ЧЕРЕЗ ENTRIES ЗРОБИТИ ЦЕ!
}
...
function filterObjectsByKey(arr, key) {
  ...
  const filtered = arr.filter((element) => element.hasOwnProperty(key))
...
function convertArrayToObj(arr) {
  ...
  if (!Array.isArray(arr)) {
    return {}; // Повертаємо пустий об'єкт, якщо вхідний параметр не є масивом
  }

  const obj = {};

  for (let i = 0; i < arr.length; i++) {
    const [key, value] = arr[i]; // Розпаковка поточного підмасиву на ключ та значення

    // Перевірка, чи ключ вже існує в об'єкті
    if (obj.hasOwnProperty(key)) {
      console.log(`У масиві вже є дубльований ключ: ${key}`);
    }

    obj[key] = value; // Додаємо ключ та значення до об'єкта
  }

  return obj; // Повертаємо сформований об'єкт
}
